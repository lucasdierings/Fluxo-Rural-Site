...
9. **Decap CMS usa `publish_mode: editorial_workflow`** (`public/admin/config.yml`) — criar/editar post no `/admin` NAO commita direto na main, abre uma PR (branch `cms/blog/...`) que fica **pendente ate alguem clicar em "Publish"/merge no proprio Decap CMS (ou merge manual no GitHub)**. Post nao aparece no site ate isso acontecer + o rebuild automatico do Cloudflare Pages terminar.
10. **MDX interpreta `{...}` como expressao JS** — formulas estilo LaTeX (`\frac{CFT}{PV}`) ou qualquer `{variavel}` solto no corpo do artigo quebra o `next build` inteiro (`ReferenceError: X is not defined`), derrubando o export estatico e travando o deploy de TODOS os posts, nao so do novo. Em `content/blog/*.mdx`, formulas/variaveis devem ir em inline code (`` `PEA = (CFT + DF) / (PV - CV)` ``), nunca com `{}` cru. `MDXRemote` em `app/blog/[slug]/page.tsx` usa `remark-gfm` (tabelas, checklists `- [ ]`, etc.) — nao usa `remark-math`/KaTeX, entao notacao matematica deve ser texto/codigo simples, nao LaTeX.
11. **`public/sitemap.xml`, `public/sitemap-0.xml` e `public/robots.txt` sao gerados pelo `postbuild` (next-sitemap) direto em `/out`** (ver comentario em `next-sitemap.config.js`), NUNCA devem ser commitados em `public/` — ficam no `.gitignore`. Se aparecerem versionados de novo, sao lixo de build antigo (antes do fix de `outDir: 'out'`) e podem ser removidos.
