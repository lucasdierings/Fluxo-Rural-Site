...
- [X] Campanha Google Ads com R$ 880
- [X] Google Search Console — verificar indexacao
...
## Sessão 17 jul/2026 — Estruturação e Ativação do Google Ads (Calculadora e Palestras)

Concluímos o planejamento estratégico de marketing e efetuamos a criação e ativação direta via API do Google Ads das campanhas na conta de anúncios.

### Campanhas e Configurações Criadas via API (Conta 8239842688)

- **Campanha B2C (Calculadora):** `Fluxo Rural - B2C - Calculadora de Margem` (ID: `24046312699`)
  - **Status:** **PAUSADO** (Orçamento diário: R$ 10,00 | CPC Manual inicial: R$ 2,00).
  - **Objetivo:** Geração de Leads e Banco de Dados (Inbound). Produtores de Soja, Milho e Trigo.
  - **Negativas de Pecuária:** 20 termos negativos (como `gado`, `leite`, `corte`, `boi`, `vaca`, `pasto`, etc.) configurados no nível de campanha.
  - **Anúncios RSA:** Criados e apontando para `/calculadora/`.
  - **Rastreamento:** Conversão 100% amarrada via evento `generate_lead` integrado no `app.js` da calculadora (gtag para GA4 e dataLayer para GTM).
- **Campanha B2B (Palestras e Eventos):** `Fluxo Rural - B2B - Palestras e Eventos` (ID: `24046312702`)
  - **Status:** **ATIVADA (ENABLED)** (Orçamento diário: R$ 10,00 | CPC Manual inicial: R$ 4,00).
  - **Objetivo:** Contratação de palestras e workshops do Lucas Dierings para cooperativas/feiras.
  - **Geotargeting de Precisão (Raio):** Cadastrados via API 13 pontos de **raio de 3 km** ao redor das coordenadas GPS exatas das sedes das federações agrícolas estaduais (incluindo CNA, FAEP, FARSUL, FAMATO, FAEG, FAEMG, FAESP, FAMASUL, FAEB, FAESC, FAET, FAEMA e FAEPA).
  - **Anúncios RSA:** Criados e apontando para `/palestras/`.

### Lembrete de Orçamento

- Cadastrado lembrete no aplicativo nativo **Lembretes (Reminders)** e um evento correspondente no **Calendário (Calendar) do macOS** para o dia **23 de julho de 2026, às 09:00**, para revisão dos orçamentos da Beweather e do eProdutor.

**Ultima atualizacao:** 17 de julho de 2026 (18:00)
**Status:** Campanha B2B de Palestras ATIVADA e com segmentação cirúrgica por raio de 3km no Google Ads. Campanha B2C da Calculadora configurada e pausada. Benchmark da Safra implementado com estratégia dual de naming (SEO vs Visual), loop de indicação municipal ("Destravar Cidade"), copy dinâmico de WhatsApp, e e-mails do produtor automatizados no backend.

---

## Sessão 17 jul/2026 (Segundo Turno) — Naming Strategy, City Unlock e Resend Backend
...
- **Estratégia Dual de Naming:**
  ...
  - **Share Card Canvas:** Atualizado para renderizar a marca **"BENCHMARK DA SAFRA"** na imagem PNG gerada.
- **Loop de Growth Hacking ("Destravar Cidade"):**
  - **Média Municipal:** Implementada contagem de leads com LGPD consentido na mesma cidade (`n_cidade`). Ao atingir 5 ou mais, o sistema revela a margem média municipal real no gráfico e nas tabelas do produtor.
  - **Barra de Progresso:** Adicionado componente visual de progresso dinâmico na tela final mostrando quantos cadastros faltam para liberar a média local (ex: "3 de 5 produtores").
  - **WhatsApp Sharing Dinâmico:** Links de compartilhamento agora geram textos personalizados conforme os resultados (Top 25% performático, convite de engajamento de cidade para destravar média local, ou convite genérico detalhado).
- **Backend (Cloudflare Pages Function & Resend):**
  - Atualizado `functions/api/calculadora.js` para suportar salvamento estruturado dos campos novos (`custo_comercializacao`, `custo_modo`, `custos_detalhe`, `propriedade` e `interesse_gestao`).
  - Adicionado envio automático de e-mail ao produtor com o PDF/HTML do diagnóstico completo via Resend (`sendEmailProdutor`).
